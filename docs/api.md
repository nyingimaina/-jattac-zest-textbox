...
| `onChange`    | `(event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void` | `undefined` | Standard React `onChange` event handler. **Important:** For optimal performance, ensure this function is memoized (e.g., using `useCallback` or defining as a class method) to prevent unnecessary re-renders. See [Best Practices: Memoizing Function Props](./best-practices.md) for details. Note that `ZestTextbox` also provides a type-safe `onTextChanged` callback within `ZestProps` for handling parsed and validated values. If both `onChange` and `zest.onTextChanged` are provided, both will be triggered.                                                                                                                                                                                               |
...
**Important Note on Memoization:** For optimal performance and to prevent unnecessary re-renders, any function-type properties within `ZestProps` (e.g., `onTextChanged`, `parser`, `validator`, `helperTextConfig`'s `formatter` or `templater`) should be memoized using `useCallback` in functional components or defined as class methods in class components. See [Best Practices: Memoizing Function Props](./best-practices.md) for a detailed explanation and examples.
...
|--------------------------|-----------------------------------------------------------------|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `helperTextConfig`       | `ZestConfigValue<HelperTextConfig>`                             | `undefined`  | **Use Case:** Display dynamic instructions, formatted values, or validation messages below the input. <br/><br/> **Behavioral Notes:** <br/> - **Rendering Logic:** At least one of `formatter` or `templater` must be provided in the config object for helper text to render. If both are absent (e.g., returning `{}` from a config function), the component will render nothing. <br/> - **Context Access:** The `templater` function within `HelperTextConfig` receives a `context` object where `inputType` is accessible via `context.props.type`. |
| `onTextChanged`          | `(value: T | undefined) => void`                                | `undefined`  | **Use Case:** A type-safe way to get the *parsed and validated* value from the component whenever it changes. <br/><br/> **Behavioral Notes:** <br/> - **Stability:** Internally stabilized using `useRef`; passing unmemoized functions will no longer cause infinite re-render loops (though memoization is still recommended for performance). <br/> - **Invalid States:** Fires with `undefined` when the input fails validation, allowing parents to track and react to invalid states. <br/> - **Change Guard:** Only fires when the *parsed* value actually changes (verified via deep equality). <br/> - **Mount Behavior:** Skips the initial render and only fires on subsequent value changes. |
...
> **Note:** At least one of `formatter` or `templater` must be provided for the helper text to be rendered. If both are absent (e.g., passing `{}`), the component will correctly render nothing.
